...
*Setup instructions for a Gitian build of Lotuscoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Lotuscoin
...
- [Building Lotuscoin Core](#building-lotuscoin-core)
...
Clone the git repositories for lotuscoin and Gitian.
...
in the lotuscoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Lotuscoin Core
----------------

To build Lotuscoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the lotuscoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/lotuscoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/lotuscoin-project/lotuscoin
...
For example:
```bash
URL=https://github.com/thrasher-/lotuscoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit lotuscoin=${COMMIT} --url lotuscoin=${URL} ../lotuscoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit lotuscoin=${COMMIT} --url lotuscoin=${URL} ../lotuscoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit lotuscoin=${COMMIT} --url lotuscoin=${URL} ../lotuscoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the lotuscoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../lotuscoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/lotuscoin
SIGPATH=/some/root/path/lotuscoin-detached-sigs

./bin/gbuild --url lotuscoin=${BTCPATH},signature=${SIGPATH} ../lotuscoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/lotuscoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/lotuscoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/lotuscoin-osx-build.assert
```
...
[gitian.sigs.ltc](https://github.com/lotuscoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
