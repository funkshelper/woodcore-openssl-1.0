...
Below are some notes on how to build Lotuscoin Core for Windows.
...
Note that for WSL v1 the Lotuscoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/lotuscoin, AND not under, for example, /mnt/d/lotuscoin. 
...
way. This will install to `c:\workspace\lotuscoin`, for example:

    make install DESTDIR=/mnt/c/workspace/lotuscoin